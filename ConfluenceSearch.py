...
from PyQt5.QtWidgets import (
    ...
    QComboBox, QSpinBox, QVBoxLayout, QHBoxLayout, QTabWidget,
    QFileDialog, QMessageBox, QProgressBar, QFormLayout, QCheckBox,
...
def fetch_spaces(base_url: str, auth) -> list:
    ...
    while True:
        r = requests.get(url, params={"limit": limit, "start": start},
                         auth=auth, timeout=30)
        r.raise_for_status()
        data = r.json()
        out.extend({"key": s["key"], "name": s["name"]}
...
def fetch_pages(base_url: str, space_key: str, auth) -> list:
    ...
    while True:
        params = {"spaceKey": space_key, "limit": limit, "start": start,
                  "expand": "body.storage"}
...
class ConfluenceSearch(QWidget):
    ...
    def _build_ui(self):
        ...
        self.only_title  = QCheckBox("Just title"); self.only_title.setChecked(True)
        ...
        # ────────── Search tab ──────────
        t_search = QWidget(); sv = QVBoxLayout()
        ...
        vbtns.addWidget(self.btn_load_sel); vbtns.addWidget(self.btn_load_all)
        avail_layout.addLayout(vbtns)
        sv.addLayout(avail_layout)

        # section: query line
        q_line = QHBoxLayout()
        self.query = QLineEdit(); self.query.returnPressed.connect(self.do_search)
        ...
        q_line.addWidget(QLabel("Query:")); q_line.addWidget(self.query, stretch=1)
        q_line.addWidget(QLabel("Top-k:")); q_line.addWidget(self.top_k)
    ...
    def load_spaces(self):
        ...
        QMessageBox.information(self, "Spaces",
                                f"{self.space_box.count()} spaces loaded.")
    ...
    def load_all_available_indexes(self):
        keys = [self.available_list.item(i).text()
    ...
    def fetch_and_index(self):
        ...
        space_key = self.space_box.currentData() or \
                    self.space_box.currentText().split(" — ")[0]
        ...
        texts = [title if self.only_title.isChecked()
                 else f"{title}\n{body}"
        ...
        index = faiss.IndexIVFFlat(quantizer, dim,
                                   self.nlist.value(),
        ...
        pickle.dump({i: (pid, title, url)
                     for i, (pid, title, _, url) in enumerate(pages)},
                    open(map_path, "wb"))

        self._log(f"Index built and saved ({idx_path}).")
        QMessageBox.information(self, "Done",
                                f"Indexed {len(pages)} pages.")
        self._refresh_available_indexes()
    ...
    def do_search(self):
        if not self.loaded_indices:
            QMessageBox.warning(self, "No index",
                                "Load or build at least one index.")
            return
        ...
        vec = self._lazy_model().encode([query], convert_to_numpy=True)\
                                .astype("float32")
